...
const HomePage = () => {
  ...
  useEffect(() => {
    // Hero section animation - using a safer approach that doesn't interfere with styles
    if (heroRef.current) {
      // Using more controlled animations that won't conflict with styling
      ...
      // Use fromTo instead of from to have more control
      if (heroTitle) {
        gsap.fromTo(heroTitle, { opacity: 0 }, { opacity: 1, duration: 0.5, ease: 'power2.out' });
      }
      
      if (heroDesc) {
        gsap.fromTo(heroDesc, { opacity: 0 }, { opacity: 1, duration: 0.5, delay: 0.1, ease: 'power2.out' });
      }
      
      if (heroButtons.length > 0) {
        gsap.fromTo(heroButtons, { opacity: 0 }, { opacity: 1, stagger: 0.1, duration: 0.5, delay: 0.2, ease: 'power2.out' });
      }
      
      if (heroImage) {
        gsap.fromTo(heroImage, { opacity: 0 }, { opacity: 1, duration: 0.5, delay: 0.3, ease: 'power2.out' });
      }
    }

    // Features section animation - using safer approach
    if (featuresRef.current && featureRefs.current.length > 0) {
      ...
      featureRefs.current.forEach((feature, index) => {
        if (feature) {
          gsap.fromTo(feature, 
            { opacity: 0 }, 
            { opacity: 1, duration: 0.5, delay: index * 0.1, ease: 'power2.out',
              scrollTrigger: {
                trigger: feature,
                start: 'top 85%',
                toggleActions: 'play none none none'
    ...
    // How it works section animation - safer approach
    if (howItWorksRef.current && stepRefs.current.length > 0) {
      const sectionTitle = howItWorksRef.current.querySelector('.section-title');
      ...
      stepRefs.current.forEach((step, index) => {
        if (step) {
          // Only animate opacity, not position properties which can conflict with layout
          gsap.fromTo(step,
            { opacity: 0 },
            { opacity: 1, duration: 0.5, delay: index * 0.1, ease: 'power2.out',
              scrollTrigger: {
    ...
    <div className="min-h-screen">
          ...
          <div className="space-y-16">
            <div ref={addToStepRefs} className="flex flex-col md:flex-row items-center">
              ...
              <div className="md:w-1/2">
                <img src="/images/meal-plan-selection.svg" alt="Choose Your Meal Plan" width="300" height="300" className="rounded-lg shadow-lg" />
              </div>
            </div>
            ...
            <div ref={addToStepRefs} className="flex flex-col md:flex-row-reverse items-center">
              ...
              <div className="md:w-1/2">
                <img src="/images/browse-order.svg" alt="Browse & Order" width="300" height="300" className="rounded-lg shadow-lg" />
              </div>
            </div>
            ...
            <div ref={addToStepRefs} className="flex flex-col md:flex-row items-center">
              ...
              <div className="md:w-1/2">
                <img src="/images/receive-delivery.svg" alt="Receive Your Delivery" width="300" height="300" className="rounded-lg shadow-lg" />
              </div>
            </div>
            ...
            <div ref={addToStepRefs} className="flex flex-col md:flex-row-reverse items-center">
              ...
              <div className="md:w-1/2">
                <img src="/images/enjoy-repeat.svg" alt="Enjoy & Repeat" width="300" height="300" className="rounded-lg shadow-lg" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-indigo-700 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Link to="/register" className="bg-white hover:bg-gray-100 text-indigo-600 font-bold py-3 px-8 rounded transition-colors duration-200 text-lg">
            Sign Up Now
          </Link>
        </div>
      </div>
